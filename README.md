...
正式实例的安装、升级、回退、卸载和 Danmuku Web 入口操作集中在 [部署说明](deploy/README.md)，Docker 生产模板位于 [deploy/docker/prod](deploy/docker/prod/)。目标路径、账号和网络入口由部署环境决定；开发脚本仅用于隔离测试。本项目不打包 Jellyfin Server，也不维护自定义 Server 镜像。
